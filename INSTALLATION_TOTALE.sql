-- ==========================================
-- INSTALLATION COMPLÈTE DU SYSTÈME ACADÉMIQUE
-- ==========================================
...
-- 2. Table de suivi Coranique (avec contrainte UNIQUE)
CREATE TABLE IF NOT EXISTS student_quran_tracking (
    ...
    CONSTRAINT student_quran_tracking_student_id_key UNIQUE (student_id)
);

-- 3. Table de suivi Multi-Livres
...
-- ==========================================
-- POLITIQUES DE SÉCURITÉ (RLS) & GRANTS
-- ==========================================

-- Accorder les permissions de base aux rôles Supabase
GRANT ALL ON student_quran_tracking TO authenticated, anon, service_role;
GRANT ALL ON student_books_tracking TO authenticated, anon, service_role;
GRANT ALL ON academic_evaluations TO authenticated, anon, service_role;
GRANT ALL ON juz_sourates TO authenticated, anon, service_role;

-- Activer RLS
ALTER TABLE student_quran_tracking ENABLE ROW LEVEL SECURITY;
ALTER TABLE student_books_tracking ENABLE ROW LEVEL SECURITY;
ALTER TABLE academic_evaluations ENABLE ROW LEVEL SECURITY;
ALTER TABLE juz_sourates ENABLE ROW LEVEL SECURITY;

-- Créer les politiques (si elles n'existent pas)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all for authenticated users' AND tablename = 'student_quran_tracking') THEN
        CREATE POLICY "Allow all for authenticated users" ON student_quran_tracking FOR ALL TO authenticated USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all for authenticated users' AND tablename = 'student_books_tracking') THEN
        CREATE POLICY "Allow all for authenticated users" ON student_books_tracking FOR ALL TO authenticated USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all for authenticated users' AND tablename = 'academic_evaluations') THEN
        CREATE POLICY "Allow all for authenticated users" ON academic_evaluations FOR ALL TO authenticated USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all for authenticated users' AND tablename = 'juz_sourates') THEN
        CREATE POLICY "Allow all for authenticated users" ON juz_sourates FOR ALL TO authenticated USING (true);
    END IF;
END;
$$;
